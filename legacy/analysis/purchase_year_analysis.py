...
def create_purchase_year_scenario(
    base_scenario: EconomicScenario, purchase_year: int
) -> EconomicScenario:
    """
    ...
    def extend_trajectory(
        trajectory: List[float], default_growth: float = 0.0
    ) -> List[float]:
    ...
    adjusted_scenario = EconomicScenario(
        ...
        diesel_price_trajectory=extend_trajectory(
            base_scenario.diesel_price_trajectory, 0.03
        ),
        electricity_price_trajectory=extend_trajectory(
            base_scenario.electricity_price_trajectory, 0.02
        ),
        battery_price_trajectory=extend_trajectory(
            base_scenario.battery_price_trajectory, -0.07
        ),
        carbon_price_trajectory=extend_trajectory(
            base_scenario.carbon_price_trajectory, 0.0
        ),
        bev_efficiency_improvement=extend_trajectory(
            base_scenario.bev_efficiency_improvement, -0.02
        ),
        diesel_efficiency_improvement=extend_trajectory(
            base_scenario.diesel_efficiency_improvement, -0.01
        ),
        maintenance_cost_multiplier=extend_trajectory(
            base_scenario.maintenance_cost_multiplier, 0.0
        ),
...
def calculate_adjusted_vehicle_price(
    vehicle: VehicleModel, purchase_year: int, base_scenario: EconomicScenario
) -> float:
    """
    ...
    if vehicle.drivetrain_type == "BEV" and years_offset > 0:
        ...
        if years_offset < len(base_scenario.battery_price_trajectory):
            battery_cost_multiplier = base_scenario.battery_price_trajectory[
                years_offset
            ]
        else:
            # Extrapolate battery cost decline
            battery_cost_multiplier = base_scenario.battery_price_trajectory[-1] * (
                0.93 ** (years_offset - len(base_scenario.battery_price_trajectory) + 1)
            )
        ...
        adjusted_price = vehicle.msrp * (
            non_battery_portion + battery_portion * battery_cost_multiplier
        )
...
def analyse_purchase_years(start_year: int = 2024, end_year: int = 2030) -> Dict:
    ...
    for purchase_year in range(start_year, end_year + 1):
        ...
        for vehicle in target_vehicles:
            ...
            adjusted_price = calculate_adjusted_vehicle_price(
                vehicle, purchase_year, base_scenario
            )
            ...
            year_results[vehicle.vehicle_id] = {
                ...
                "price_change_percent": ((adjusted_price - vehicle.msrp) / vehicle.msrp)
                * 100,
        ...
        bev_tcos = [
            data["total_tco"]
            for vid, data in year_results.items()
...
def create_summary_tables(results: Dict) -> str:
    ...
    for year, data in results["summary_by_year"].items():
        ...
        summary.append(
            f"| {year} | ${bev_tco:,.0f} | ${diesel_tco:,.0f} | ${data['avg_bev_cost_per_km']:.2f} | ${data['avg_diesel_cost_per_km']:.2f} | {advantage:.1f}% |"
        )
        ...
        if spec["drivetrain_type"] == "BEV":
            ...
            for year in range(2024, 2031):
                if year in results["purchase_year_analysis"]:
                    price = results["purchase_year_analysis"][year][vehicle_id][
                        "adjusted_msrp"
                    ]
    ...
    for year in sorted(results["bev_vs_diesel_savings"].keys()):
        for comparison_id, data in results["bev_vs_diesel_savings"][year].items():
            summary.append(
                f"| {year} | {data['bev_model'][:15]} vs {data['diesel_model'][:15]} | "
...
def main():
    ...
    for year in [2024, 2026, 2028, 2030]:
        if year in results["summary_by_year"]:
            ...
            advantage = (
                ((diesel_avg - bev_avg) / diesel_avg * 100) if diesel_avg > 0 else 0
            )

            print(
                f"{year}: BEV ${bev_avg:,.0f} vs Diesel ${diesel_avg:,.0f} "
    ...
    bev_vehicle = next(
        v
        for v in results["vehicle_specifications"].values()
    ...
    for year in [2024, 2026, 2028, 2030]:
        if year in results["purchase_year_analysis"]:
            price = results["purchase_year_analysis"][year][bev_vehicle["vehicle_id"]][
                "adjusted_msrp"
            ]
