...
def generate_executive_summary(
    ...
    summary["overview"] = {
        ...
        "percentage_competitive": (
            (len(bev_cheaper) / total_vehicles * 100) if total_vehicles > 0 else 0
        ),
        "average_additional_cost": (
            np.mean([diff for _, _, diff in comparisons if diff > 0])
            if any(diff > 0 for _, _, diff in comparisons)
            else 0
        ),
        "average_savings": (
            -np.mean([diff for _, _, diff in comparisons if diff < 0])
        ...
        "best_case_savings": (
            -min(diff for _, _, diff in comparisons) if comparisons else 0
        ),
        "worst_case_cost": (
            max(diff for _, _, diff in comparisons) if comparisons else 0
        ),
    }
    ...
    for weight_class, class_comparisons in by_class.items():
        ...
        if best_pair[2] < 0:  # Only if BEV is cheaper
            ...
            summary["by_class"][weight_class]["best_performing_pair"] = {
                ...
                "payback_years": (
                    payback.payback_years if payback.breakeven_achieved else None
                ),
            }
    ...
    best_class = min(
        summary["by_class"].items(), key=lambda x: x[1]["avg_tco_difference"]
    )
...
def generate_fleet_report(
    fleet_composition: Dict[str, int], scenario_name: str = "Current"
) -> pd.DataFrame:
    """
    ...
    for vehicle_id, quantity in fleet_composition.items():
        ...
        if vehicle.comparison_pair and vehicle.comparison_pair in BY_ID:
            ...
            diesel_emissions = (
                diesel_inputs.vehicle.litres_per_km
                * diesel_inputs.vehicle.annual_kms
                * const.DIESEL_EMISSIONS
                / 1000
            )  # tonnes
            ...
            report_data.append(
                {
                    ...
                    "quantity": quantity,
                    "unit_capex_premium": bev_inputs.initial_cost
                    - diesel_inputs.initial_cost,
                    "fleet_capex_premium": (
                        bev_inputs.initial_cost - diesel_inputs.initial_cost
                    )
                    * quantity,
                    "unit_annual_savings": diesel_tco.annual_cost - bev_tco.annual_cost,
                    "fleet_annual_savings": (
                        diesel_tco.annual_cost - bev_tco.annual_cost
                    )
                    * quantity,
                    "payback_years": (
                        payback.payback_years if payback.breakeven_achieved else None
...
def generate_policy_recommendations(
    current_results: Dict[str, TCOResult], target_adoption_rate: float = 0.5
) -> List[Dict]:
    """
    ...
    if current_rate < target_adoption_rate:
        ...
        target_reduction = (
            np.percentile(all_gaps, required_percentile) if all_gaps else 0
        )
