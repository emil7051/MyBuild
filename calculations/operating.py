...
class FuelCostCalculator:
    ...
    def __init__(
        self, vehicle: VehicleModel, scenario: Optional[EconomicScenario] = None
    ):
    ...
    def get_fuel_cost_year(
        ...
        if self.vehicle.drivetrain_type == "BEV":
            ...
            price_multiplier = (
                self.scenario.get_electricity_price_multiplier(year)
                if self.scenario
                else 1.0
            )
            ...
            if (
                self.scenario
                and self.scenario.bev_efficiency_improvement
                and year <= len(self.scenario.bev_efficiency_improvement)
            ):
                efficiency_multiplier = self.scenario.bev_efficiency_improvement[
                    year - 1
                ]
            ...
            # Get charging proportions from the new dictionary structure
            proportions = const.CHARGING_MIX_PROPORTIONS["BEV"][
                self.vehicle.weight_class
            ]
            ...
            # Diesel vehicle
            price_multiplier = (
                self.scenario.get_diesel_price_multiplier(year)
                if self.scenario
                else 1.0
            )
            ...
            if (
                self.scenario
                and self.scenario.diesel_efficiency_improvement
                and year <= len(self.scenario.diesel_efficiency_improvement)
            ):
                efficiency_multiplier = self.scenario.diesel_efficiency_improvement[
                    year - 1
                ]
            ...
            adjusted_litres_per_km = self.vehicle.litres_per_km * efficiency_multiplier
            base_cost = (
                adjusted_litres_per_km * self.vehicle.annual_kms * const.DIESEL_PRICE
            )
...
class ChargingTimeCostCalculator:
    ...
    def calculate_annual_charging_labour_cost(self) -> float:
        ...
        if daily_kms <= usable_range:
            ...
            daily_charging_sessions = math.ceil(
                (daily_kms - usable_range) / usable_range
            )
        ...
        annual_charging_hours = (
            daily_charging_sessions * avg_charging_hours * working_days
        )
...
class MaintenanceCostCalculator:
    ...
    def get_annual_base_cost(self) -> float:
        ...
        cost_per_km = const.MAINTENANCE_COST_PER_KM[self.vehicle.drivetrain_type][
            self.vehicle.weight_class
        ]
    ...
    def get_maintenance_cost_year(
        ...
        multiplier = (
            self.scenario.get_maintenance_cost_multiplier(year)
            if self.scenario
            else 1.0
        )
...
class BatteryReplacementCalculator:
    ...
    def get_replacement_cost_year8(self) -> float:
        ...
        if (
            self.vehicle.drivetrain_type != "BEV"
            or self.vehicle.battery_capacity_kwh == 0
        ):
            return 0.0
    ...
    def get_battery_replacement_year(
        ...
        if year == 8 and self.vehicle.drivetrain_type == "BEV":
            ...
            if overrides and "battery_life_variation" in overrides:
                ...
                multiplier = (
                    const.BATTERY_LIFE_VARIATION_BASE
                    - overrides["battery_life_variation"]
                )  # If life is 0.7x, cost is 1.3x
...
def calculate_carbon_cost_year(
    ...
    annual_emissions = (
        vehicle.litres_per_km * vehicle.annual_kms * const.DIESEL_EMISSIONS / 1000
    )
