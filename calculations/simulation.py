...
class MonteCarloSimulation:
    ...
    def run(
        self, iterations: int = 10000, seed: Optional[int] = None
    ) -> SimulationResults:
        ...
        for i in range(iterations):
            ...
            sample_overrides = {
                param.override_key: param.sample() for param in self.parameters.values()
            }

            # 2. Calculate TCO directly, passing the base inputs and the new overrides
            from .calculations import calculate_tco_from_inputs

            tco = calculate_tco_from_inputs(
                self.base_inputs, overrides=sample_overrides
            )
            tco_results.append(tco.total_cost)

        return SimulationResults(
            iterations=iterations, tco_values=np.array(tco_results)
        )

    def compare_uncertainty(
        self, other_inputs: VehicleInputs, iterations: int = 10000
...
class SensitivityAnalysis:
    ...
    def analyse_parameter(
        ...
        for value in values:
            ...
            percent_change = (
                (new_tco.total_cost - self.base_tco.total_cost)
                / self.base_tco.total_cost
                * 100
            )
