...
class FinancingCalculator:
    ...
    def calculate_total_financing_cost(
        monthly_payment: float, loan_amount: float
    ) -> float:
...
class DepreciationCalculator:
    ...
    def __init__(
        self, initial_cost: float, scenario: Optional[EconomicScenario] = None
    ):
    ...
    def get_depreciation_year(
        self, year: int, drivetrain_type: str = "Diesel"
    ) -> float:
        ...
        if (
            drivetrain_type == "BEV"
            and self.scenario
            and self.scenario.bev_residual_value_multiplier
            and year <= len(self.scenario.bev_residual_value_multiplier)
        ):
