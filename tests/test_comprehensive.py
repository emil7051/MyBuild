...
from data.scenarios import SCENARIOS, EconomicScenario, create_custom_scenario
...
class TestFinancialCalculations:
    ...
    def test_present_value_calculations(self):
        ...
        expected_pv = sum(
            annual_amount / (1 + discount_rate) ** i for i in range(1, years + 1)
        )
...
class TestTCOCalculations:
    ...
    def test_bev_vs_diesel_comparison(self):
        ...
        assert (
            bev_tco.purchase_cost > diesel_tco.purchase_cost
        )  # BEVs typically more expensive upfront
    ...
    def test_scenario_impact(self):
        ...
        tech_breakthrough_tco = calculate_tco(
            vehicle, SCENARIOS["technology_breakthrough"]
        )
    ...
    def test_carbon_pricing_impact(self):
        ...
        carbon_scenario = create_custom_scenario(
            ...
            electricity_price_trajectory=SCENARIOS[
                "baseline"
            ...
            diesel_efficiency_improvement=SCENARIOS[
                "baseline"
...
class TestNPVFinancing:
    ...
    def calculate_manual_npv(
        self, initial_cost, down_payment_rate, interest_rate, term_years, discount_rate
    ):
        ...
        if loan_amount > 0:
            ...
            npv_monthly_payments = calculate_npv_of_payments(
                monthly_payment, num_payments, discount_rate
            )
    ...
    def test_npv_calculation_standard(self):
        ...
        vehicle = BY_ID["BEV001"]
        inputs_financed = vehicle_data.get_vehicle(
            vehicle.vehicle_id, SCENARIOS["baseline"], "financed"
        )
    ...
    def test_edge_cases(self):
        ...
        try:
            ...
            inputs = vehicle_data.get_vehicle(
                vehicle.vehicle_id, SCENARIOS["baseline"], "financed"
            )
...
class TestMonteCarloSimulation:
    ...
    def test_simulation_comparison(self):
        ...
        assert (
            abs(np.mean(differences) - (bev_results.mean - diesel_results.mean)) < 1.0
        )
    ...
    def test_sensitivity_analysis(self):
        ...
        results = sensitivity.analyse_parameter(
            "electricity_price", [0.8, 1.0, 1.2], parameter_type="multiplier"
        )
...
class TestIntegration:
    ...
    def test_complete_bev_diesel_comparison(self):
        ...
        assert (
            abs(diesel_results.mean - diesel_tco.total_cost) / diesel_tco.total_cost
            < 0.2
        )
    ...
    def test_scenario_workflow(self):
        ...
        scenario_results = calculate_scenario_comparison(
            vehicle_id, list(SCENARIOS.values())[:3]
        )
        ...
        breakeven_results = calculate_breakeven_analysis(
            vehicle_id, diesel_id, list(SCENARIOS.values())[:3]
        )
