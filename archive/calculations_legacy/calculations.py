...
from .inputs import VehicleInputs, vehicle_data
from .utils import (
    calculate_annualised_cost,
    calculate_npv_of_annual_cashflows,
...
class TCOResult:
    ...
    residual_value: float = (
        0.0  # Present value of residual value at end of vehicle life
    )
...
def calculate_tco_from_inputs(
    vehicle_inputs: VehicleInputs, overrides: Optional[Dict[str, float]] = None
) -> TCOResult:
    ...
    if vehicle_inputs.purchase_method == "outright":
        ...
        financing_cost = 0.0
        npv_purchase_payments = (
            vehicle_inputs.initial_cost
        )  # Full amount paid in year 0
    else:
        ...
        npv_monthly_payments = calculate_npv_of_payments(
            vehicle_inputs.monthly_payment, num_payments, const.DISCOUNT_RATE
        )
    ...
    residual_value_future = vehicle_inputs.get_residual_value(
        const.VEHICLE_LIFE, overrides
    )
    ...
    total_registration_undiscounted = (
        vehicle_inputs.vehicle.annual_registration * const.VEHICLE_LIFE
    )
    taxes_and_fees = vehicle_inputs.stamp_duty + total_registration_undiscounted

    # Generate lists of annual costs over vehicle life
    annual_fuel_costs = [
        vehicle_inputs.get_fuel_cost_year(year, overrides)
        for year in range(1, const.VEHICLE_LIFE + 1)
    ]
    annual_battery_costs = [
        vehicle_inputs.get_battery_replacement_year(year, overrides)
        for year in range(1, const.VEHICLE_LIFE + 1)
    ]
    annual_carbon_costs = [
        vehicle_inputs.get_carbon_cost_year(year, overrides)
        for year in range(1, const.VEHICLE_LIFE + 1)
    ]
    annual_maintenance_costs = [
        vehicle_inputs.get_maintenance_cost_year(year, overrides)
        for year in range(1, const.VEHICLE_LIFE + 1)
    ]
    annual_charging_labour_costs = [
        vehicle_inputs.get_charging_labour_cost_year(year, overrides)
        for year in range(1, const.VEHICLE_LIFE + 1)
    ]
    annual_payload_penalties = [
        vehicle_inputs.get_payload_penalty_year(year, overrides)
        for year in range(1, const.VEHICLE_LIFE + 1)
    ]
    ...
    total_charging_labour_cost = calculate_npv_of_annual_cashflows(
        annual_charging_labour_costs
    )
    ...
    total_insurance_pv = calculate_present_value(
        vehicle_inputs.annual_insurance_cost, const.VEHICLE_LIFE
    )
    ...
    annual_cost = calculate_annualised_cost(
        total_cost, const.VEHICLE_LIFE, const.DISCOUNT_RATE
    )
    ...
    return TCOResult(
        ...
        * const.VEHICLE_LIFE,  # Undiscounted for reporting
...
def calculate_tco(
    ...
    vehicle_inputs = vehicle_data.get_vehicle(
        vehicle.vehicle_id, scenario, purchase_method
    )
    return calculate_tco_from_inputs(vehicle_inputs)


def calculate_all_tcos(
    scenario: Optional[EconomicScenario] = None,
    purchase_method: Literal["outright", "financed"] = "financed",
) -> Dict[str, TCOResult]:
    """Calculate TCO for all vehicles in the database."""
    results = {}
    for vehicle_id, vehicle_inputs in vehicle_data.get_all_vehicles(
        scenario, purchase_method
    ).items():
        results[vehicle_id] = calculate_tco_from_inputs(vehicle_inputs)
    return results


def compare_vehicle_pairs(
    scenario: Optional[EconomicScenario] = None,
    purchase_method: Literal["outright", "financed"] = "financed",
) -> List[Tuple[TCOResult, TCOResult, float]]:
    """Compare TCO between BEV and diesel pairs, returning cost difference."""
    comparisons = []

    for bev_inputs, diesel_inputs in vehicle_data.get_vehicle_pairs(
        scenario, purchase_method
    ):
