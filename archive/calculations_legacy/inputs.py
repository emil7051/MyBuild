...
class VehicleInputs:
    ...
    def __post_init__(self):
        ...
        self._maintenance_calculator = MaintenanceCostCalculator(
            self.vehicle, self.scenario
        )
        self._insurance_calculator = InsuranceCostCalculator(self.vehicle)
        self._battery_calculator = BatteryReplacementCalculator(
            self.vehicle, self.scenario
        )
        ...
        self.stamp_duty = calculate_stamp_duty(
            self.vehicle.msrp, self.vehicle.drivetrain_type == "BEV"
        )
        self.rebate = calculate_rebate(self.vehicle.msrp, self.vehicle.drivetrain_type)
        self.initial_cost = calculate_initial_cost(
            self.vehicle.msrp, self.vehicle.drivetrain_type
        )

        # Depreciation calculator (needs initial cost)
        self._depreciation_calculator = DepreciationCalculator(
            self.initial_cost, self.scenario
        )
        self.first_year_depreciation = (
            self._depreciation_calculator.first_year_depreciation
        )
        self.annual_depreciation_rate = (
            self._depreciation_calculator.annual_depreciation_rate
        )

        # Financing calculations (only if financed)
        if self.purchase_method == "financed":
            self.down_payment = self._financing_calculator.calculate_down_payment(
                self.initial_cost
            )
            self.loan_amount = self._financing_calculator.calculate_loan_amount(
            ...
            self.total_financing_cost = (
                self._financing_calculator.calculate_total_financing_cost(
        ...
        self.annual_maintenance_cost = (
            self._maintenance_calculator.get_annual_base_cost()
        )
        self.annual_vehicle_insurance = (
            self._insurance_calculator.get_vehicle_insurance()
        )
        self.annual_insurance_cost = self._insurance_calculator.get_total_insurance()

        # Battery replacement cost (year 8 for BEVs)
        self.battery_replacement_cost_year8 = (
            self._battery_calculator.get_replacement_cost_year8()
        )

        # Charging labour cost (BEV only)
        self.annual_charging_labour_cost = (
            self._charging_calculator.calculate_annual_charging_labour_cost()
        )

        # Payload penalty
        self.annual_payload_penalty = (
            self._payload_calculator.calculate_annual_payload_penalty()
        )

    def get_fuel_cost_year(
        self, year: int, overrides: Optional[Dict[str, float]] = None
    ) -> float:
    ...
    def get_battery_replacement_year(
        self, year: int, overrides: Optional[Dict[str, float]] = None
    ) -> float:
    ...
    def get_depreciation_year(self, year: int) -> float:
        ...
        return self._depreciation_calculator.get_depreciation_year(
            year, self.vehicle.drivetrain_type
        )

    def get_residual_value(
        self, year: int, overrides: Optional[Dict[str, float]] = None
    ) -> float:
        """Get residual value at the end of a specific year."""
        residual_value = self._depreciation_calculator.get_residual_value(
            year, self.vehicle.drivetrain_type
        )
    ...
    def get_charging_labour_cost_year(
        self, year: int, overrides: Optional[Dict[str, float]] = None
    ) -> float:
    ...
    def get_annual_kms(self, overrides: Optional[Dict[str, float]] = None) -> float:
        ...
        if overrides and "annual_kms_variation" in overrides:
            ...
            ]  # Return the absolute value from the override
...
class VehicleData:
    ...
    def _initialise_all_vehicles(self):
        ...
        for vehicle_id, vehicle in BY_ID.items():
            self._inputs_cache[vehicle_id] = VehicleInputs(
                vehicle, self._default_scenario, self._default_purchase_method
            )
    ...
    def get_vehicle(
        ...
        vehicle_id: str,
        scenario: Optional[EconomicScenario] = None,
        purchase_method: Optional[Literal["outright", "financed"]] = None,
    ) -> VehicleInputs:
    ...
    def get_vehicle_pairs(
        self,
        scenario: Optional[EconomicScenario] = None,
        purchase_method: Optional[Literal["outright", "financed"]] = None,
    ) -> List[tuple[VehicleInputs, VehicleInputs]]:
        ...
        for vehicle_id, vehicle_inputs in all_vehicles.items():
            if (
                vehicle_inputs.vehicle.drivetrain_type == "BEV"
                and vehicle_inputs.vehicle.comparison_pair in all_vehicles
            ):
                bev = vehicle_inputs
