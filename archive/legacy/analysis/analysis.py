...
def calculate_payback_analysis(
    bev_inputs: VehicleInputs, diesel_inputs: VehicleInputs
) -> PaybackAnalysis:
    """
    ...
    for year in range(1, const.VEHICLE_LIFE + 1):
        ...
        if payback_year is None and diesel_cumulative > bev_cumulative:
            ...
            if year == 1:
                ...
                prev_diff = (
                    cumulative_bev_costs[year - 2] - cumulative_diesel_costs[year - 2]
                )
            ...
            for stamp_duty in policy_options["stamp_duty_exemption"]:
                for loan_subsidy in policy_options["green_loan_subsidy"]:
                    for carbon in policy_options["carbon_price"]:
                        ...
                        policies.POLICIES["stamp_duty_exemption"].enabled = (
                            stamp_duty > 0
                        )
                        policies.POLICIES[
                            "stamp_duty_exemption"
                        ].exemption_percentage = stamp_duty

                        policies.POLICIES["green_loan_subsidy"].enabled = (
                            loan_subsidy > 0
                        )
                        policies.POLICIES["green_loan_subsidy"].rate_reduction = (
                            loan_subsidy
                        )
                        ...
                        baseline_diff = get_baseline_tco_difference(
                            scenario, purchase_method
                        )
                        tco_improvement = baseline_diff - avg_difference
                        cost_effectiveness = policy_cost / max(
                            tco_improvement, 1
        ...
        if years_offset > 0:
            ...
            for attr in [
                ...
                if len(trajectory) > years_offset:
                    ...
                    setattr(
                        adjusted_scenario, attr, [trajectory[-1]] * const.VEHICLE_LIFE
                    )
            ...
            vehicle_inputs = vehicle_data.get_vehicle(
                vehicle_id, adjusted_scenario, purchase_method
            )
        else:
            vehicle_inputs = vehicle_data.get_vehicle(
                vehicle_id, base_scenario, purchase_method
            )
...
def estimate_annual_policy_cost(
    ...
    annual_cost += (
        avg_bev_price * const.STAMP_DUTY_RATE * stamp_duty_exemption
    ) * estimated_bev_sales
    ...
    if loan_subsidy > 0:
        ...
        annual_cost += (
            subsidy_cost_per_vehicle * estimated_bev_sales / const.FINANCING_TERM
        )
