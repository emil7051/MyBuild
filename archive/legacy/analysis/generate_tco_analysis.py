...
from app import VehicleInputs, calculate_all_tcos, vehicle_data
...
def generate_year_by_year_costs(
    vehicle_inputs: VehicleInputs, start_year: int = 2024
) -> Dict:
    ...
    for year in range(1, VEHICLE_LIFE + 1):
        ...
        yearly_breakdown[calendar_year] = {
            ...
            "battery_replacement_cost": vehicle_inputs.get_battery_replacement_year(
                year
            ),
...
def generate_scenario_summary(scenario_name: str = "baseline") -> Dict:
    ...
    return {
        ...
        "cost_multipliers": {
            ...
            "bev_residual_value_multiplier": getattr(
                scenario, "bev_residual_value_multiplier", [1.0] * 15
            ),
        },
        "policy_parameters": {
...
def generate_policies_summary() -> Dict:
    ...
    policy_summary = {"active_policies": {}, "all_policies": {}}
...
def main():
    ...
    for vehicle in target_vehicles:
        print(
            f"  - {vehicle.vehicle_id}: {vehicle.model_name} ({vehicle.drivetrain_type}, {vehicle.weight_class})"
        )
    ...
    for vehicle in target_vehicles:
        ...
        vehicle_inputs = vehicle_data.get_vehicle(
            vehicle.vehicle_id, scenario, "financed"
        )
        ...
        analysis_results["yearly_cost_breakdowns"][vehicle.vehicle_id] = (
            generate_year_by_year_costs(vehicle_inputs, 2024)
        )
    ...
    print("-" * 60)
    print(
        f"{'Vehicle ID':<12} {'Model':<25} {'Type':<8} {'Total TCO':<12} {'Annual':<10} {'$/km':<8}"
    )
    print("-" * 60)
    ...
    for vehicle_id, tco in analysis_results["tco_results"].items():
        ...
        print(
            f"{vehicle_id:<12} {vehicle.model_name[:24]:<25} {vehicle.drivetrain_type:<8} "
