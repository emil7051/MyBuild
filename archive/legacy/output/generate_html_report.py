        ...
        class_data[vehicle.weight_class].append(
            {
                ...
                "difference": diff,
                "savings_pct": (
                    -diff / diesel_tco.total_cost * 100
                    if diesel_tco.total_cost > 0
                    else 0
                ),
            }
        )
...
def generate_monte_carlo_differentials(vehicle_pairs, scenario):
    ...
    for i, (bev_tco, diesel_tco, cost_diff) in enumerate(vehicle_pairs):
        ...
        print(
            f"  Simulating {bev_inputs.vehicle.model_name} vs {diesel_inputs.vehicle.model_name}..."
        )
        ...
        tco_differential_std = (
            bev_results.std_dev**2 + diesel_results.std_dev**2
        ) ** 0.5
        ...
        result = {
            ...
            "probability_bev_cheaper": sum(
                1
                for b, d in zip(bev_results.tco_values, diesel_results.tco_values)
                if b < d
            )
            / len(bev_results.tco_values),
        }
    ...
    for weight_class, results in weight_class_results.items():
        ...
        avg_probability = sum(r["probability_bev_cheaper"] for r in results) / len(
            results
        )
...
def create_monte_carlo_differentials_chart(monte_carlo_results, weight_class_averages):
    ...
    fig = make_subplots(
        ...
        row_heights=[0.7, 0.3],
        subplot_titles=(
            "TCO Differentials by Vehicle Pair (Diesel TCO - BEV TCO)",
    ...
    vehicle_labels = [
        f"{r['bev_model']} vs {r['diesel_model']}" for r in sorted_results
    ]
    ...
    avg_differentials = [
        weight_class_averages[wc]["avg_tco_differential"] for wc in weight_classes
    ]
...
def generate_comprehensive_report(
    scenario_name: str = "Current", output_file: str = "tco_report.html"
):
    ...
    monte_carlo_differentials, weight_class_averages = (
        generate_monte_carlo_differentials(comparisons, scenario)
    )
    ...
    charts["class_comparison"] = generate_class_comparison_chart(
        comparisons, scenario_name
    )

    # 3. Monte Carlo results
    charts["monte_carlo"] = generate_monte_carlo_chart(
        mc_results, f"Monte Carlo Simulation: {sample_vehicle.vehicle.model_name}"
    )
    ...
    charts["monte_carlo_differentials"] = create_monte_carlo_differentials_chart(
        monte_carlo_differentials, weight_class_averages
    )
    ...
    for bev_tco, diesel_tco, cost_diff in comparisons:
        ...
        diesel_vehicle = vehicle_data.get_vehicle(
            diesel_tco.vehicle_id, scenario
        ).vehicle
        ...
        row_class = (
            "positive-savings"
            if cost_diff < 0
            else "negative-savings" if cost_diff > 0 else ""
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate comprehensive TCO HTML report"
    )
    parser.add_argument(
        "--scenario",
        default="baseline",
    ...
    parser.add_argument(
        "--output", default="tco_report.html", help="Output HTML file name"
    )
