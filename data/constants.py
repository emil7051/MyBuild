...
# 
# NOTE: The following constants are now superseded by the scenario system in scenarios.py:
# - ELECTRICITY_PRICE_INCREASE: Use scenario.electricity_price_trajectory instead
# - DIESEL_PRICE_INCREASE: Use scenario.diesel_price_trajectory instead  
# - BATTERY_ANNUAL_PRICE_DROP: Use scenario.battery_price_trajectory instead
#
# These constants are kept for backward compatibility but the scenario system
# should be used for all new calculations requiring time-varying parameters.
...
ELECTRICITY_PRICE_INCREASE = 0.02  # %/year (DEPRECATED - use scenarios.py)

# Electricity Mix

RETAIL_PROPORTION = 0.0  # % (Proportion of charging done at workplace AC chargers)
OFFPEAK_PROPORTION = 1.0  # % (Proportion of charging done at workplace AC chargers during offpeak hours)
SOLAR_PROPORTION = 0.0  # % (Proportion of charging done at home via solar and storage)
PUBLIC_PROPORTION = 0.0  # % (Proportion of charging done at public DC fast chargers)
...
DIESEL_PRICE_INCREASE = 0.03  # % (DEPRECATED - use scenarios.py)
...
BATTERY_ANNUAL_PRICE_DROP = 0.07  # % (DEPRECATED - use scenarios.py)
